...
def normalize(raw):
    ...
    if dz1.get("wspolnicySpzoo"):
        ...
        wlasc_lista = [
            normalize_wlasciciel(w, "czyPosiadaCaloscUdzialow") for w in dz1["wspolnicySpzoo"]
        ]
    elif dz1.get("jedynyAkcjonariusz"):
        wlasc_typ = "jedyny_akcjonariusz"
        wlasc_lista = [
            normalize_wlasciciel(w, "czyPosiadaCaloscAkcji") for w in dz1["jedynyAkcjonariusz"]
        ]
    else:
    ...
    pkd_kod_parts = [
        pkd_main.get("kodDzial"),
        pkd_main.get("kodKlasa"),
        pkd_main.get("kodPodklasa"),
    ]
...
def render_md(dataset):
    ...
    for entry in dataset:
        ...
        adres_str = (
            f"{a.get('ulica') or ''} {a.get('nr_domu') or ''}{lokal},"
            f" {a.get('kod_pocztowy') or ''} {a.get('miejscowosc') or ''},"
            f" {a.get('kraj') or ''}"
        ).strip()
        ...
        lines.append(
            f"- **Stan na dzien:** {m['stan_z_dnia']}"
            f" (wpis #{m['nr_ostatniego_wpisu']} z {m['data_ostatniego_wpisu']})"
        )
        lines.append("")
        lines.append(
            f"**Zarzad** — organ: `{z['nazwa_organu']}`,"
            f" osob: {len(z['sklad'])}, reprezentacja: _{z['sposob_reprezentacji']}_"
        )
        lines.append("")
        ...
        if not w["lista"]:
            lines.append(
                "- _brak danych w KRS"
                " (typowe dla SA bez jedynego akcjonariusza — dane w ksiedze akcyjnej)_"
            )
        for wl in w["lista"]:
