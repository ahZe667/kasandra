...
def render_md(dataset):
    ...
    for entry in dataset:
        ...
        if status == "ok":
            ...
            lines.append(
                f"- **Zgłoszenie:** `{m['zgloszenie_id']}`{korekta_flag},"
                f" ważne {m['data_poczatku_prezentacji']} – {m['data_konca_prezentacji']}"
            )
            ...
            for b in beneficjenci:
                ...
                if b["charakter_udzialu_kod"] == "1":
                    szczegol = (
                        f"{b['udzial_ilosc']} {b['udzial_jednostka_opis']}"
                        f" — {b['rodzaj_wlasnosci_opis']}"
                    )
                ...
                lines.append(
                    f"- `{b['osoba_key']}` ({b['obywatelstwo']}/{b['panstwo_zamieszkania']})"
                )
...
def main():
    ...
    for xml_path in sorted(snap_dir.glob("*.xml")):
        ...
        print(
            f"OK: {xml_path.name} -> {out_name} ({len(normalized['beneficjenci'])} beneficjentow)"
        )
