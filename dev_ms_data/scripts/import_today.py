...
def import_date(date: str) -> None:
    ...
    # VAT
    vat_dir = normalized_root / "vat"
    vat_count = 0
    if vat_dir.exists():
        for json_path in sorted(f for f in vat_dir.glob("*.json") if not f.name.startswith("_")):
            with open(json_path, encoding="utf-8") as fp:
                payload = json.load(fp)
            nip = payload["company"].get("nip")
            krs = NIP_TO_KRS.get(nip) if nip else None
            if krs is None:
            ...
            if row is None:
                print(f"  WARN: KRS {krs} nie w companies, pomijam")
                continue
            has_subject = payload["vat"].get("status") is not None
            status = "ok" if has_subject else "brak_wpisu"
            actual_payload = payload if has_subject else None
            snap_id = insert_snapshot(
                conn,
                company_id=row["id"],
                source="vat",
        ...
        print(f"  SKIP: brak {vat_dir}")

    conn.commit()
    conn.close()
    print(f"\nGotowe: {krs_count} KRS + {vat_count} VAT snapshots dla {date}")
